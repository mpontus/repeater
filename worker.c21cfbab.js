...
},{"symbol-observable":"node_modules/symbol-observable/es/index.js"}],"node_modules/xstream/extra/dropRepeats.js":[function(require,module,exports) {
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
...
},{"../index":"node_modules/xstream/index.js"}],"src/worker/CircularBuffer.ts":[function(require,module,exports) {
"use strict";
...
var CircularBuffer = /** @class */function () {
    ...
    CircularBuffer.prototype.readLast = function (length) {
...
},{}],"src/worker/VoiceDetector.ts":[function(require,module,exports) {
"use strict";
...
},{}],"src/worker/index.ts":[function(require,module,exports) {
"use strict";

Object.defineProperty(exports, "__esModule", {
    value: true
});
exports.BUFFER_SIZE = undefined;
 ...
 */
var BUFFER_SIZE = exports.BUFFER_SIZE = 2646000;
/**
 * Create a stream which emits at
 */
var createVoiceDetectorStream = function createVoiceDetectorStream(action$, sampleRate, silenceDuration, amplitudeThreshold) {
    var voiceDetector = new _VoiceDetector2.default(amplitudeThreshold, silenceDuration * sampleRate);
    action$.filter((0, _utils.ofType)("process")).subscribe({
        next: function next(action) {
            return voiceDetector.write(action.data);
        }
    });
    action$.filter((0, _utils.ofType)("update_settings")).subscribe({
        next: function next(action) {
    ...
    return action$.map(function () {
        return voiceDetector.isHearingVoice();
    }).compose((0, _dropRepeats2.default)());
};
/**
 * Create a stream which
 */
var createSpeechCaptureStream = function createSpeechCaptureStream(action$, sampleRate, contextDuration, isHearingVoice$) {
    var buffer = new _CircularBuffer2.default(BUFFER_SIZE);
    action$.filter((0, _utils.ofType)("process")).subscribe({
        next: function next(action) {
            return buffer.write(action.data);
        }
    });
    return isHearingVoice$.filter((0, _utils.is)(true)).map(function () {
        var recordingLength = contextDuration * sampleRate;
        action$.filter((0, _utils.ofType)("process")).subscribe({
            next: function next(action) {
                recordingLength += action.data.length;
            }
        });
        return isHearingVoice$.filter((0, _utils.is)(false)).map(function () {
            return buffer.readLast(recordingLength);
        }).take(1);
    }).flatten();
};
/**
 ...
 */
var createStreamProcessor = function createStreamProcessor(action$) {
    return action$.filter((0, _utils.ofType)("start")).map(function (action) {
        var _a = action.data,
            sampleRate = _a.sampleRate,
            silenceDuration = _a.silenceDuration,
            amplitudeThreshold = _a.amplitudeThreshold,
            contextDuration = _a.contextDuration;
        var isHearingVoice$ = createVoiceDetectorStream(action$, sampleRate, silenceDuration, amplitudeThreshold);
        var capturedSentence$ = createSpeechCaptureStream(action$, sampleRate, contextDuration, isHearingVoice$);
        return _xstream2.default.merge(isHearingVoice$.filter((0, _utils.is)(true)).mapTo({ key: "voice_start" }), capturedSentence$.map(function (data) {
            return { key: "voice_end", data: data };
        })).endWhen(action$.filter((0, _utils.ofType)("stop")));
    }).flatten();
};
var ctx = self;
var action$ = _xstream2.default.create({
    start: function start(listener) {
        ctx.onmessage = function (e) {
            return listener.next(e.data);
        };
    },
    stop: function stop() {
        ctx.onmessage = null;
...
},{"xstream":"node_modules/xstream/index.js","xstream/extra/dropRepeats":"node_modules/xstream/extra/dropRepeats.js","./CircularBuffer":"src/worker/CircularBuffer.ts","./VoiceDetector":"src/worker/VoiceDetector.ts","../utils":"src/utils.ts"}],"node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
var global = arguments[3];
...
},{}]},{},["node_modules/parcel-bundler/src/builtins/hmr-runtime.js","src/worker/index.ts"], null)
//# sourceMappingURL=/worker.c21cfbab.map